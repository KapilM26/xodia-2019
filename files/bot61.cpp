...
void AllValidMoves(string (&)[20]);
void makeMove(string);
bool MyBotColour;
string botMove();
int Move_Weight(string);
void Mini_Max_0(int (&)[20],string (&)[20] );
int Mini_Max_0_1(int (&)[20],string (&)[20]);
...
void Board:: Mini_Max_0(int (&Move_value)[20],string (&validmoves)[20])
{
	int y;
    for(y=0;y<20;y++)
    if(validmoves[y]=="fuh")
...
int Board::Mini_Max_1(Board x,string ryan)
{
	...
	for(int u=0;u<y;u++)
	{
	 ...
	 for (Z=0;Z<y;Z++)
		{
			avg=avg+Move_value[Z];
		}
		  	
 	return Move_Weight(ryan)-avg;
...
int Board::Mini_Max_2(Board t,string ryan)
{
	...
	for (int Z=0;Z<j;Z++)
		{
			avg=avg+t.Move_Weight(validMoves[Z]);
		}
...
int Board:: Mini_Max_0_1(int (&Move_value)[20],string (&validmoves)[20])
{
...
void Board:: AllValidMoves(string (&ValidMoves)[20])
{
	string sample;
	sample="     \n";
	int k=0;
