...
def startLeague():
    ...
    while iterator1 < len(query_set_list) - 1:
        ...
        res = compileTest1(qs_obj1.user_obj.pk, qs_obj1.bot_ext)
        if not res is None:
            print str(query_set_list[iterator1].user_obj.pk) + " Eliminated"
            query_set_list.pop(iterator1)
            continue
        iterator2 = iterator1 +1
        while iterator2 < len(query_set_list):
            qs_obj2 = query_set_list[iterator2]
            res = compileTest2(qs_obj2.user_obj.pk, qs_obj2.bot_ext)
            if not res is None:
                print str(query_set_list[iterator2].user_obj.pk) + " Eliminated"
                query_set_list.pop(iterator2)
                continue
            print qs_obj1.pk, qs_obj2.pk
            req_obj = SandboxRequest(
            user1_id=qs_obj1.user_obj.pk,
            user2_id=qs_obj2.user_obj.pk,
